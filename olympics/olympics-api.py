...
def get_nocs():
    ...
    try:
        cursor = connection.cursor()
        query = '''SELECT DISTINCT team_NOC.NOC, team_NOC.team
                   FROM team_NOC
...
def get_medalists(games_id, noc):
    ...
    for row in cursor:
        temp_dic = {row[0]: {"Name": row[1], "Sex": row[2], "Sport": row[3], "Event": row[4], "Medal": row[5]} }
